''' 
Created by Jonathan L Chu, 2020 for Metis SF20_DS18
Module to download and process
MTA turnstiles data for use in pandas

Version used for MTA_Explore_Weekdays.ipynb
'''
...
def processTurnstiles(df):
    ...
    df['DOF'] = pd.Categorical(df['DATETIME'].dt.day_name(),categories=weekdays)
    df['DOF_SUMM'] = df['DOF'].apply(lambda x: "Weekend" if x in ['Saturday','Sunday'] else "Weekday")
    
    #[weekdays[df['DATETIME'][1].weekday()] for dstring in df.DATE.tolist()]
